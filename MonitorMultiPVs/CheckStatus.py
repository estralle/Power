...
outputname = {
    ...
    "LEBT_VC03HPS01": "LEBT:VC03HPS01::Alarm:Tmp",
...
# 依次运行每个 camonitor 命令
monitor_duration = 10  # 每个 camonitor 运行的持续时间（秒）

def run_camonitor(command, value, duration, log_file):
    process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    
    def read_output(process, value, log_file):
        while True:
            output = process.stdout.readline()
            if output == '' and process.poll() is not None:
                break
            if output:
                log_file.write(f"Output from {value}: {output.strip()}\n")
                print(f"Output from {value}: {output.strip()}")
    
    # 启动线程读取输出
    thread = threading.Thread(target=read_output, args=(process, value, log_file))
    thread.start()
    ...
    if stderr:
        log_file.write(f"Error from {value}: {stderr.strip()}\n")
        print(f"Error from {value}: {stderr.strip()}")
    
    log_file.write("\n" + "="*40 + "\n\n")

with open(log_file_path, 'a') as log_file:
    for key, value in outputname.items():
        command = f"camonitor {value}"
        run_camonitor(command, value, monitor_duration, log_file)